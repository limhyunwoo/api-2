...
public class AddDemo {
	public static void main(String[] args) {
		 ...
		 * 
		 * Iterator를 사용하면 대부분 while문도 같이 써준다.
		 * 그리고 printf 출력문으로 대부분 사용함.
		 */
		...
		// 인터페이스<제너릭> 참조변수 = new 구현한 객체 <제너릭>();
		// 패턴입니다.
		list2.addAll(list2); // 서울, 부산, 대구 를 복사해서 집어 넣은 상태
		
		/* 
		 *  ☆ 출력 4 . 확장 for 문
		 *  콜렉션은 인덱스에 대한 조건이 굳이 주어지지 않은
		...
		while (it3.hasNext()) {
			str = it3.next();
			if(str.startsWith("대")){  // "대" 라고 시작하는 문자열 검색
		...
		while (it4.hasNext()) {
			...
			// 참고로 printf에서 
			// 10진수 숫자같은 %d로 d는 decimal 의 약자이다.
		}
		System.out.println();
	}
}
